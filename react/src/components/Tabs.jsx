...
export default function BasicTabs({ company, setCompanyId }) {
  ...
  return companyData.map((c) => (
    <>
      ...
      >
        <Stack direction="row" sx={{ gap: "1em", p: 4 }}>
          <Avatar
            variant="square"
            src={`${c.attributes.image.data.attributes.formats.thumbnail.url}`}
            sx={{ m: 1, width: 70, height: 70 }}
          ></Avatar>
          {/* </Grid> */}
        ...
        <Stack direction="row" sx={{ m: 4, gap: "0.5em" }}>
          <Button
            sx={{
              bgcolor: "#5A85C2",
            }}
            onClick={() => openInNewTab(`https://www.${c.attributes.name}.com`)}
            variant="contained"
            startIcon={<ArrowOutwardIcon />}
          >
            Visit Website
      ...
      >
        ...
        >
          <List>
            ...
            <ListItemText sx={{ textAlign: "center", mt: 10 }}>
          ...
          {value === 5 &&
            ...
            c.attributes.reviews.data.map((r) => (
              <>
                ...
                <Rating name="read-only" value={r.attributes.stars} readOnly />{" "}
                <br />
                <Typography variant="p" sx={{ my: 3 }} gutterBottom>
                  {r.attributes.feedback && r.attributes.feedback}
                </Typography>
